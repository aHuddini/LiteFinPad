...
class DataImporter:
    ...
    def __init__(self, expense_tracker, status_callback=None):
        """
        ...
        try:
            ...
            if not is_valid:
                messagebox.showerror(
                    config.Messages.TITLE_IMPORT_ERROR,
                ...
                if self.status_callback:
                    self.status_callback("Import failed: Invalid file format", config.StatusBar.ERROR_ICON)
            ...
            if not is_valid_integrity:
                ...
                # Update status bar if callback provided
                if self.status_callback:
                    self.status_callback("Import failed: File integrity check failed", config.StatusBar.ERROR_ICON)
            ...
            if success:
                ...
                messagebox.showinfo(
                    config.Messages.TITLE_IMPORT_SUCCESS,
                ...
                if self.status_callback:
                    self.status_callback(
                        f"Import successful: {backup_data['total_expenses']} expenses imported", 
                        config.StatusBar.SUCCESS_ICON
                    )
...
def import_expense_backup(expense_tracker, filepath: Optional[str] = None, status_callback=None):
    """
    ...
    Args:
        ...
        status_callback: Optional callback function for status updates (message, icon)
    """
    importer = DataImporter(expense_tracker, status_callback)
